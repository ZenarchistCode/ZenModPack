class ZenCampSiteManager extends Managed
{
	// Define required variables
	ref Timer m_CampSiteTimer;
	vector m_LastCamp;

	// Create Camp Site manager process
	void ZenCampSiteManager()
	{
		m_CampSiteTimer = new Timer();
		m_CampSiteTimer.Run(GetZenCampSitesConfig().SpawnSecs, this, "ManageCamps", NULL, true);
	}

	void ~ZenCampSiteManager()
	{
		if (m_CampSiteTimer && m_CampSiteTimer.IsRunning())
			m_CampSiteTimer.Stop();
	}
