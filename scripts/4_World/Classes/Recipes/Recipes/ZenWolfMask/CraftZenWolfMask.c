class CraftZenWolfMask extends RecipeBase
{
	override void Init()
	{
		m_Name = "#craft " + "Wolf Mask";
		...
		// GDZ Animal Heads
		if (GetGame().ConfigIsExisting("CfgVehicles WolfHead"))
		{
			InsertIngredient(0, "WolfHead"); 
			InsertIngredient(0, "WolfTrophy");
		}
		else
		// If no GDZ Animal Heads mod found, just use pelt
		{
			InsertIngredient(0, "WolfPelt");
		}
		...
		//result1
		AddResult("ZenWolfMask");//add results here
	...
	{
		return ZenModEnabled("ZenWolfMask") && super.CanDo(ingredients, player);
	}
};