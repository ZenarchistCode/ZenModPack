class CraftZenCamoShelterKit extends RecipeBase
{
    override void Init()
    {
        m_Name = "#STR_CraftZenCamoShelterKit";
        ...
        //result1
        AddResult("Zen_CamoShelterKit");//add results here
    ...
    {
        return super.CanDo(ingredients, player) && ZenModEnabled("ZenCamoCamp");
    }
    ...
    override void Do(ItemBase ingredients[], PlayerBase player, array<ItemBase> results, float specialty_weight)//gets called upon recipe's completion
    {
        if (results.Count() > 0)
        {
            EntityAI rope = results.Get(0).GetInventory().CreateInInventory("Rope");
            rope.SetHealth(ingredients[1].GetHealth());
        }
    }
};
