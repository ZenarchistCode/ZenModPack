class Zen_CraftChessPieceBase extends RecipeBase
{
	string GetPieceName() { return ""; }

	override void Init()
	{
		m_Name = "#craft #STR_ZenChess" + GetPieceName();
		...
		//result1
		//AddResult("Zen_ChessBoard");//add results here
    ...
    {
        return super.CanDo(ingredients, player) && ZenModEnabled("ZenChess");
    }

	override void Do(ItemBase ingredients[], PlayerBase player, array<ItemBase> results, float specialty_weight)//gets called upon recipe's completion
	{
		string pieceType;
		if (ingredients[0].IsInherited(Bark_Oak))
			pieceType = "Zen_ChessBlack" + GetPieceName();
		else
			pieceType = "Zen_ChessWhite" + GetPieceName();

		GetGame().CreateObjectEx(pieceType, player.GetPosition(), ECE_PLACE_ON_SURFACE);
	}
};
