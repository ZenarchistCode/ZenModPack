class ActionZenRepairPumpCB : ActionContinuousBaseCB
{
...
class ActionZenRepairPump : ActionContinuousBase
{
	void ActionZenRepairPump()
	{
		m_CallbackClass = ActionZenRepairPumpCB;
	...
	override void OnFinishProgressServer(ActionData action_data)
	{
		...
		int PumpIndex = GetZenPumpsDB().GetRepairablePumpIndex(pumpObj.GetPosition());
		GetZenPumpsDB().SetPumpRepaired(PumpIndex, pumpObj.GetPosition(), true, true); // If index == -1 then a new Pump will be saved.
