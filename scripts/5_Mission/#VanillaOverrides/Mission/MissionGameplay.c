modded class MissionGameplay
{
	...
	protected int m_EarPlugsVolume = 50; // 50% volume by default
	...
	void UpdateZenEarPlugs()
	{
		...
		if (GetGame().GetInput().LocalPress("UAZenEarPlugsToggle"))
		{
			...
			{
				m_EarPlugsOn = true;
				GetGame().GetSoundScene().SetSoundVolume(m_EarPlugsVolume / 100, 0.5); // Set volume to X% over X second delay.
				m_EarPlugsWidget.ShowIcon();
		...
		if (m_EarPlugsOn)
		{
			if (GetGame().GetInput().LocalPress("UAZenEarPlugsIncreaseVol")) // Increase volume key (+)
			{
				...
				if (m_EarPlugsVolume > 75)
				{
					m_EarPlugsVolume = 75;
				}

				GetGame().GetSoundScene().SetSoundVolume(m_EarPlugsVolume / 100, 0.5);
				m_EarPlugsWidget.SetVolume(m_EarPlugsVolume);
			}
